...
import { GET_EMPLOYEEQR_DATA } from 'Constants/utils';
const ViewEmployeeQr = () => {
...
console.log(employees,"___________");
const [isQrPopupOpen, setIsQrPopupOpen] = useState(false);
const [currentQrCode, setCurrentQrCode] = useState(null);
const [isLoadingQr, setIsLoadingQr] = useState(false);

const handleQrCodeClick = async (employeeId) => {
  setIsLoadingQr(true);
  try {
    const response = await fetch(`${GET_EMPLOYEEQR_DATA}${employeeId}/qrcode`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
console.log(response,"llllll__");
    const imageBlob = await response.blob();
    const imageUrl = URL.createObjectURL(imageBlob);
    setCurrentQrCode(imageUrl);
    console.log(currentQrCode,"kkk");
...
// Close QR popup and clean up
const closeQrPopup = () => {
  setIsQrPopupOpen(false);
  if (currentQrCode) {
    URL.revokeObjectURL(currentQrCode); // Clean up memory
    ...
    <div className="p-4 bg-white mt-[30px] ml-8 mr-8 mb-8">
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Name Of Employee</th>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {employees.map((employee, index) => (
              <tr key={index}>
                ...
                <td className="px-6 py-4 text-sm text-gray-700 whitespace-nowrap">{employee?.email}</td>
                <td className="px-6 py-4 text-sm text-gray-700 whitespace-nowrap"><IoQrCode size={25} onClick={()=>handleQrCodeClick(employee.id) } disabled={isLoadingQr}/></td>
      ...
      {isQrPopupOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          {/* Popup Content */}
          <div className="relative z-10 bg-white p-6 rounded-lg shadow-xl max-w-sm">
            <button
            ...
            <h3 className="text-lg font-medium mb-4">Employee QR Code</h3>
            {currentQrCode ? (
              <img 
                src={currentQrCode} 
